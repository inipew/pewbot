...
func New(cfg Config, engine *taskengine.Service, log *slog.Logger, bus eventbus.Bus) *Service {
	if log == nil {
		log = slog.Default()
	}
	return &Service{
...
//
// NOTE: This service is trigger-only in Phase 2; execution happens in taskengine.Service.
func (s *Service) Start(ctx context.Context) {
	...
	cur := s.cfg
	s.log.Debug("start requested", slog.Bool("enabled", cur.Enabled), slog.String("tz", strings.TrimSpace(cur.Timezone)))
	...
	s.log.Info("service started", slog.String("tz", loc.String()), slog.Int("schedules", len(s.defs)))
}

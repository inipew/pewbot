...
func looksLikeNumber(s string) bool {
	...
	// Fast path for typical negatives like -1, -0.5
	if s[0] == '-' && len(s) > 1 {
		_, err := strconv.ParseFloat(s, 64)
		return err == nil
	}
	// Also accept positives to avoid special-casing.
	_, err := strconv.ParseFloat(s, 64)
	return err == nil
}
...
func parseFlags(args []string) (pos []string, flags map[string]string, bools map[string]bool) {
	...
	for i := 0; i < len(args); i++ {
		a := args[i]
		if a == "--" {
			// Terminator: everything after this is positional.
		...
		if strings.HasPrefix(a, "--") && len(a) > 2 {
			...
			// value in next token?
			if i+1 < len(args) && (!strings.HasPrefix(args[i+1], "-") || looksLikeNumber(args[i+1])) {
				flags[key] = args[i+1]
				i++
				continue
			}
